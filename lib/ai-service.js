...
const aiService = {
  /**
   * Generate an itinerary using an LLM (OpenAI or Claude) when configured.
   * Env:
   * - AI_PROVIDER: openai | claude (optional)
  ...
  async generateItinerary(context) {
    ...
    const fallback = await itineraryGenerator.generateItinerary(formData);
    ...
    const itinerary = normalizeItinerary(raw, fallback);
