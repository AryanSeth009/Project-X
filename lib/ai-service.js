...
async function generateWithMistral({ prompt, model }) {
  ...
  if (!resp.ok) {
    const errText = await resp.text().catch(() => '');
    throw new Error(`Mistral API error (${resp.status}): ${errText || resp.statusText}`);
  }
        ...
        activities: [
          {
            ...
            image_url: "'' (leave blank, fetched later)",
          },
        ],
      },
    ],
  ...
  async generateItinerary(context) {
    ...
    try {
      if (provider === 'gemini') {
        ...
        raw = await generateWithMistral({ prompt, model: mistralModel });
        usedProvider = raw ? 'mistral' : 'local';
      } else if (provider === 'openai') {
        raw = await generateWithOpenAI({ prompt, model: openaiModel });
        usedProvider = raw ? 'openai' : 'local';
      } else {
        // Auto-select based on which keys are present
        raw = await generateWithOpenAI({ prompt, model: openaiModel });
    ...
    if (itinerary && Array.isArray(itinerary.days)) {
      await Promise.all(itinerary.days.map(async (day) => {
        if (Array.isArray(day.activities)) {
          await Promise.all(day.activities.map(async (act) => {
            act.image_url = await itineraryGenerator.getRealImage(act, formData.destination);
          }));
        }
      }));
    }
