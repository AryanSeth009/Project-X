...
class ItineraryGenerator {
  ...
  getDestinationActivities(destination, geoContext = null) {
    if (!geoContext) return [];

    const activities = [];

    // Map popular attractions
    if (Array.isArray(geoContext.popularAttractions)) {
      geoContext.popularAttractions.forEach((attr) => {
        const avgTimeStr = attr.avgTime || '2';
        const entryFee = parseFloat(String(attr.entryFee || '0').replace(/[^0-9]/g, '')) || 0;
        activities.push({
          name: attr.name,
          cost: entryFee,
          duration: parseFloat(avgTimeStr.split('-')[0]) || 2,
          category: 'attraction',
          description: `${attr.type || 'Attraction'}${attr.bestTime ? ` - Best time: ${attr.bestTime}` : ''}`,
          location: attr.name,
          geoSpecific: true,
        });
      });
    ...
    if (Array.isArray(geoContext.specificActivities)) {
      geoContext.specificActivities.forEach((act) => {
        let duration = 2;
        if (typeof act.duration === 'string') {
          duration = /min/i.test(act.duration) ? Math.max(0.25, (parseFloat(act.duration) || 30) / 60) : parseFloat(act.duration) || 2;
        } else if (typeof act.duration === 'number') duration = act.duration;
        activities.push({
          name: act.name,
          cost: act.cost || 0,
          duration,
    ...
    // Map specific restaurants
    if (Array.isArray(geoContext.specificRestaurants)) {
      geoContext.specificRestaurants.forEach((rest) => {
        activities.push({
          name: rest.name,
          cost: rest.cost || 500,
          duration: 1.5,
          category: 'food',
          description: `${rest.type || 'Restaurant'} - ${rest.bestFor || 'Local cuisine'}`,
          location: `${(rest.area || '')} ${rest.name}`.trim(),
          geoSpecific: true,
        });
      });
    }
  ...
  async getRealImage(activity, destination = '') {
    const title = activity.name || activity.title || '';
    
    try {
      let cleanTitle = title.replace(/^(Breakfast|Lunch|Dinner) at /i, '');
      const query = `${cleanTitle} ${destination}`.trim();
      ...
      const res = await fetch(url, { headers: { 'User-Agent': 'YatraAI/1.0' }, signal: controller.signal });
    ...
    const keywords = ['aerial', 'landscape', 'landmark', 'scenic', '4k'];
    const seed = this.hashStringToUnitInterval(title) * 100;
    const k1 = keywords[Math.floor(seed) % keywords.length];
    
    // Fallback: loremflickr which redirects to valid flickr photo
    const cleanDest = encodeURIComponent((destination || category).replace(/[^a-zA-Z0-9 ]/g, '').split(' ')[0]);
    return `https://loremflickr.com/800/600/${cleanDest},${k1}`;
  }

  async generateTimeSlots(activities, destination = '', startHour = CONFIG.DAY_START_HOUR) {
    ...
    for (let index = 0; index < activities.length; index++) {
      ...
      const image_url = await this.getRealImage(activity, destination);
  ...
  async generateItinerary(formData, geoContext) {
    ...
    const allActivities = this.getDestinationActivities(destination, geoContext);
    ...
    geoBoostedActivities = allActivities.map((act) => {
      return {
        ...act,
        score: (act.score || 1) + 2, // Default boost since they all come from geo now
      };
    });
    ...
    if (formData.itineraryStyle === 'top-10') {
      ...
      const timeSlots = await this.generateTimeSlots(top10Activities, formData.destination);
    ...
    for (let day = 1; day <= days; day++) {
      ...
      const timeSlots = await this.generateTimeSlots(dayActivities, formData.destination);
